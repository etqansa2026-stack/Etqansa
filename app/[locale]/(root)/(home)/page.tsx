...
export default async function Page({ params }: PageProps) {
  ...
  const bannersData = await getAllBannersByLocale(locale)();
  const number_of_programs = await getSettingByFieldName("number_of_programs");
  const number_of_students = await getSettingByFieldName("number_of_students");
  const number_of_instructors = await getSettingByFieldName("number_of_instructors");
